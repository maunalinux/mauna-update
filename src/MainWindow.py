...
locale.bindtextdomain('mauna-update', '/usr/share/locale')
locale.textdomain('mauna-update')
...
class MainWindow(object):
    def __init__(self, application):
        ...
        self.about_dialog.set_program_name(_("Mauna Update"))
        if self.about_dialog.get_titlebar() is None:
            about_headerbar = Gtk.HeaderBar.new()
            about_headerbar.set_show_close_button(True)
            about_headerbar.set_title(_("About Mauna Update"))
            about_headerbar.pack_start(Gtk.Image.new_from_icon_name("mauna-update", Gtk.IconSize.LARGE_TOOLBAR))
    ...
    def define_variables(self):
        ...
        self.icon_available = "mauna-update-available-symbolic" if system_wide else "software-update-available-symbolic"
        self.icon_normal = "mauna-update-symbolic" if system_wide else "security-medium-symbolic"
        self.icon_inprogress = "mauna-update-inprogress-symbolic" if system_wide else "media-playlist-repeat-symbolic"
    ...
    def init_ui(self):
        ...
        if not system_wide:
            self.main_window.set_default_icon_from_file(
                os.path.dirname(os.path.abspath(__file__)) + "/../data/mauna-update.svg")
            self.about_dialog.set_logo(None)
            self.ui_uptodate_image.set_from_pixbuf(
                GdkPixbuf.Pixbuf.new_from_file_at_size(
                    os.path.dirname(os.path.abspath(__file__)) + "/../data/mauna-update-uptodate.svg", 288, 288))

    def init_indicator(self):
        self.indicator = appindicator.Indicator.new(
            "mauna-update", self.icon_normal, appindicator.IndicatorCategory.APPLICATION_STATUS)
        self.indicator.set_status(appindicator.IndicatorStatus.ACTIVE)
        self.indicator.set_title(_("Mauna Update"))
        self.menu = Gtk.Menu()
...
class Notification(GObject.GObject):
    ...
    def update_callback(self, widget, action):
        ...
        subprocess.Popen(["/usr/bin/mauna-update", "--page", "updateinfo"])
